...
int screen_width_bytes = 61; // 22; // 31;
...
{
    if(rfb_initialized && (modified_maxx - modified_minx > 0) && (modified_maxy - modified_miny > 0)) {
...
void textport_erase_line(int which)
{
    int start, end;
    ...
    if(down) {
        if(key == XK_Control_L || key == XK_Control_R) {
            ...
            if(pass_key)
                ...
                textport_character(key);
        } else if(key == XK_Return) {
            if(pass_key) {
                key = 10;
                write(pipe_to_subprocess, &key, 1);
            } else {
                textport_character('\r');
                textport_character('\n');
            }
...
{
    printf("terminal is %dx%d\n", textport_columns, textport_rows);
